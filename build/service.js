...
function getMethod(request, service, msg) {
    return hasMethod(request.method, service)
        ...
        : Either_1.left(error_1.MethodNotFound({ method: request.method }, msg));
}
exports.getMethod = getMethod;
function toFailureError(error) {
    if (error_1.isFailureError(error)) {
        return error_1.FailureError(error.code, error.message, error.data);
    }
    if (typeof error === "object" && error.stack) {
        ...
        return error_1.InternalError(errorData);
    }
    return error_1.InternalError(error);
}
...
function handleRequest(request, service) {
    ...
    return getMethod(request, service)
        ...
        .then(result => success_1.Success(result, request.id), error => failure_1.Failure(handleError(error), request.id));
}
